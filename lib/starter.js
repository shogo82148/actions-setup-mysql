    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield core.group('setup MySQL Database', () => __awaiter(this, void 0, void 0, function* () {
            ...
            if (fs.existsSync(path.join(mysql.toolPath, 'bin', 'mariadb-install-db.exe'))) {
                ...
                yield exec.exec(path.join(mysql.toolPath, 'bin', 'mariadb-install-db.exe'), [`--datadir=${baseDir}${sep}var`]);
            }
                ...
                yield exec.exec(path.join(mysql.toolPath, 'bin', 'mysql_install_db.exe'), [`--datadir=${baseDir}${sep}var`]);
            }
                ...
                yield exec.exec(path.join(mysql.toolPath, 'bin', `mysqld${binExt}`), [
                ...
                yield exec.exec(command, installArgs);
            }
        }));
        let pid = 0;
        ...
        if (rootPassword) {
            yield core.group('configure root password', () => __awaiter(this, void 0, void 0, function* () {
                yield exec.exec(path.join(mysql.toolPath, 'bin', `mysqladmin${binExt}`), [
                    `--defaults-file=${baseDir}${sep}etc${sep}my.cnf`,
                    `--user=root`,
                    `--host=127.0.0.1`,
                    `password`,
...
function verboseHelp(mysql) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            yield exec.exec(path.join(mysql.toolPath, 'bin', `mysqld${binExt}`), ['--verbose', `--help`], options);
        }
        catch (e) {
